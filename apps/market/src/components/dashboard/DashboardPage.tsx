...
function StatCard({ title, value, sub, color }: { title: string; value: string; sub?: string; color?: string }) {
  return (
    <div className="glass" style={{ borderRadius: 14, padding: '1.4rem' }}>
      ...
      <div style={{ fontWeight: 600, fontSize: '1.8rem', letterSpacing: '-0.03em', color: color ?? 'var(--text)', lineHeight: 1.1 }}>{value}</div>
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', minWidth: 0 }}>
        <div style={{ width: 32, height: 32, borderRadius: '50%', flexShrink: 0, background: `linear-gradient(135deg, ${asset.category === 'crypto' ? 'var(--gold)' : asset.category === 'stocks' ? '#00d4ff' : '#00ff88'}22, transparent)`, border: '1px solid var(--border2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600, fontSize: '0.62rem', color: 'var(--text2)' }}>
      ...
      <div style={{ fontFamily: 'Figtree, sans-serif', fontWeight: 600, fontSize: '0.88rem' }}>{formatPrice(asset.price, asset.category)}</div>
...
export function DashboardPage() {
  ...
  return (
    <div className="animate-fadeUp" style={{ maxWidth: 1200, width: '100%' }}>
        ...
        {filtered.map(asset => {
          ...
          return isForexLocked ? (
            ...
            >
              ...
              <div style={{ fontFamily: 'Figtree, sans-serif', fontWeight: 600, fontSize: '0.88rem', filter: 'blur(4px)', userSelect: 'none' }}>{'$' + asset.price.toFixed(4)}</div>
              <div style={{ filter: 'blur(4px)', userSelect: 'none', fontSize: '0.82rem' }}>±0.00%</div>
              <div style={{ fontSize: '0.72rem', color: 'var(--muted)' }}>Forex</div>
              <div><span style={{ padding: '0.2rem 0.5rem', borderRadius: 100, fontSize: '0.65rem', background: 'var(--accent-light)', color: 'var(--gold)', border: '1px solid var(--border2)' }}>Pro</span></div>
            </div>
          ) : (
      ...
      {user?.plan === 'free' && (
        <div onClick={() => navigate('/plans')} style={{ marginTop: '1.5rem', padding: '1.2rem 1.5rem', background: 'linear-gradient(135deg, var(--card2), rgba(0,212,255,0.04))', border: '1px solid rgba(201,168,76,0.18)', borderRadius: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}>
          <div>
            <div style={{ fontWeight: 600, marginBottom: '0.2rem', fontSize: '0.95rem' }}>
