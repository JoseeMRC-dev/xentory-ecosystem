  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: 'calc(var(--bar-h) + 1.5rem) clamp(1rem,4vw,2rem) 3rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,260px),1fr))', gap: 'clamp(0.8rem,3vw,1.2rem)', marginBottom: '1.5rem' }}>
        {(prefPlatform === 'sports'
          ? [
              <PlatformCard key="bets"   icon={<BetIcon />}    name="Xentory Bet"    planKey="bets"   plan={user.subscriptions.bets}   color="var(--cyan)" onLaunch={() => handleLaunch('bets')}   onUpgrade={() => navigate('/pricing?tab=bets')}   highlighted />,
              <PlatformCard key="market" icon={<MarketIcon />} name="Xentory Market" planKey="market" plan={user.subscriptions.market} color="var(--gold)"  onLaunch={() => handleLaunch('market')} onUpgrade={() => navigate('/pricing?tab=market')} />,
            ]
          : prefPlatform === 'market'
          ? [
              <PlatformCard key="market" icon={<MarketIcon />} name="Xentory Market" planKey="market" plan={user.subscriptions.market} color="var(--gold)"  onLaunch={() => handleLaunch('market')} onUpgrade={() => navigate('/pricing?tab=market')} highlighted />,
              <PlatformCard key="bets"   icon={<BetIcon />}    name="Xentory Bet"    planKey="bets"   plan={user.subscriptions.bets}   color="var(--cyan)" onLaunch={() => handleLaunch('bets')}   onUpgrade={() => navigate('/pricing?tab=bets')} />,
            ]
          : [
              <PlatformCard key="market" icon={<MarketIcon />} name="Xentory Market" planKey="market" plan={user.subscriptions.market} color="var(--gold)"  onLaunch={() => handleLaunch('market')} onUpgrade={() => navigate('/pricing?tab=market')} />,
              <PlatformCard key="bets"   icon={<BetIcon />}    name="Xentory Bet"    planKey="bets"   plan={user.subscriptions.bets}   color="var(--cyan)" onLaunch={() => handleLaunch('bets')}   onUpgrade={() => navigate('/pricing?tab=bets')} />,
